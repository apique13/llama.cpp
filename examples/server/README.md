...
- Using `CMake` with Windows service support
  ...
  ```bash
  cmake -B build -DLLAMA_SERVER_SSL=ON -DWINDOWSSERVICE=ON
  cmake --build build --config Release -t llama-server
  ```

Note for using openssl on windows : Download it from https://slproweb.com/products/Win32OpenSSL.html, install then defines before calling cmake
  ```
  set OPENSSL_ROOT_DIR=OpenSSL-Win64-dir\lib\VC\x64\MT 
  ```
